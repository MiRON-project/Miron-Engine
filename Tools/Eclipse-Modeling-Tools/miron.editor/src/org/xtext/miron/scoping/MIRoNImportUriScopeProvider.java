...
/**
 * Extension of the Xtext Gobal Scope Provider to allow using alias in the MIRoN import mechanism. 
 ...
 * <pre><code>
 * import "models/mymodel.miron" as model1
 * ...
 * // referring to a variable defined in mymodel.miron
 * model1.variable1
 * </pre></code>
 * 
 ...
 *
 */
public class MIRoNImportUriScopeProvider extends AbstractGlobalScopeProvider {
	...
	protected LinkedHashSet<ImportUriTuple> getImportedUris(final Resource resource) {
		...
		// Search the cache for instances (sets of ImportUriTuple) associated with a concrete resource. If the search returns
	...
	protected IAcceptor<ImportUriTuple> createURICollector(Resource resource, Set<ImportUriTuple> collectInto) {
		...
		return new MironURICollector(resourceSet, collectInto);
	}
	...
	{			
		...
		if(description != null) {
			...
			if(tuple.getImport() instanceof Import) {
				alias = ((Import) tuple.getImport()).getAlias();
			}
			description = new ResourceDescriptionWrapper(description, alias);
		}	
	 ...
	 *
	 */
	public static class MironURICollector implements IAcceptor<ImportUriTuple> {
		 ...
		 */
		protected MironURICollector(ResourceSet resourceSet, Set<ImportUriTuple> result) {
	...
	public class ImportUriTuple {
		...
		/**
		 * Import object in the Miron model.
		 */
		private EObject importElement;
		 ...
		 * @param importElement Miron import element.
